...
def resolve_inventory_variants(source, variant_ids, rarity_rules, number):
    ...
    holo_only=name.lower().endswith(" ex") or rarity.lower() in HOLO_ONLY_INVENTORY_RARITIES
    if holo_only:
        selected=["holo"]
    else:
        ...
        if not isinstance(selected,list) or not selected:
            raise ScaffoldError(
                "Card "+str(number)+" must provide inventory_variants, match an inventory_variant_rules rarity, "
                "or qualify for the standard Holo-only rule."
            )
...
def normalize_definition(raw, definition_path):
    ...
    return {
        ...
        "inventory_variant_rules":rarity_rules,
...
def build_schema_files(c):
    ...
    metadata={"inventoryMetadataVersion":2,"setId":c["set_id"],"source":{"kind":"generated","note":"Explicit physical inventory availability generated by Inventory Sheet Automation v2."},"cards":inventory_cards}
