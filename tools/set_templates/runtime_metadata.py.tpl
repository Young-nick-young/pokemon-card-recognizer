"""Schema v1 runtime metadata for {display_name}."""
...
EXPECTED_SET_ID = {set_id_repr}
EXPECTED_DISPLAY_CODE = {display_code_repr}
EXPECTED_DATASET_ID = {dataset_id_repr}
LEGACY_CARD_ID_KEY = "legacyInventoryCardId"
CANONICAL_PREFIX = {canonical_prefix_repr}
LEGACY_PREFIX = {legacy_prefix_repr}
...
def load_runtime_metadata(package_directory=DEFAULT_PACKAGE_DIRECTORY):
    ...
    expected_numbers = list(range(1, {expected_cards} + 1))
    if list(cards_by_number) != expected_numbers:
        raise RuntimeError("Schema v1 runtime metadata must remain ordered collector numbers 1 through {expected_cards}.")
