"""Perfect Order recognizer wrapper.

Set-specific metadata and library identity live here. Recognition behavior is
provided by the shared Schema v1 recognizer engine so future sets do not copy
the SIFT/FLANN algorithm.
"""

import os

from schema_v1_recognizer_engine import SchemaV1RecognizerEngine
from sets.perfect_order.runtime_metadata import (
...
def _sync_compatibility_state():
    global REFERENCE_CARDS, GLOBAL_DESCRIPTORS, GLOBAL_CARD_NUMBERS
    global global_matcher, library_ready, library_error
...
def configure_runtime_metadata(source=None, package_directory=None):
    global SET_ID, SET_CODE, SET_NAME, CARD_COUNT
    global RUNTIME_METADATA, RUNTIME_METADATA_SOURCE, RUNTIME_METADATA_ERROR
    global ENGINE
...
def get_runtime_card_metadata(number):
    if RUNTIME_METADATA is None:
        raise RuntimeError(
            "Perfect Order runtime metadata unavailable: "
    ...
    if metadata is None:
        raise RuntimeError(
            "Perfect Order runtime metadata has no collector number "
            + str(number)
            + "."
        )
    return metadata
...
def recognize_image(image):
    if ENGINE is None:
        return {
            "status": "error",
            "reason": "Perfect Order library unavailable",
            "library_error": RUNTIME_METADATA_ERROR,
            "top_matches": [],
        }
...
try:
    configure_runtime_metadata(source=RUNTIME_METADATA_SOURCE)
except Exception as error:
    RUNTIME_METADATA = None
    RUNTIME_METADATA_ERROR = str(error)
    _sync_compatibility_state()
    print("Perfect Order runtime metadata load failed:", error)
